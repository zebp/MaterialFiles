...
class FileGridAdapter(
    ...
    override fun getItemViewType(position: Int): Int =
        if (getItem(position).attributes.isDirectory) {
            0
        } else {
            1
        }

    fun bindDirectoryViewHolder(holder: DirectoryViewHolder, position: Int, payloads: List<Any>) {
...
//        binding.nameText.isSelected = nameEllipsize == TextUtils.TruncateAt.MARQUEE
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int, payloads: List<Any>) {
        if (holder is DirectoryViewHolder) {
            bindDirectoryViewHolder(holder, position, payloads)
    ...
    class DirectoryViewHolder(val binding: FileItemBinding) : RecyclerView.ViewHolder(binding.root) {
...
//
//    class ThumbnailViewHolder(val binding: )
